// Taken from https://github.com/RoyalAliceAcademyOfSciences/PhoneOverIp.git
	       ...
	       char* output_sms_text, int sms_text_size)
{
	...
	if (sms_deliver_start + 1 > buffer_length) return -2;
	if ((buffer[sms_deliver_start] & SMS_DELIVER_ONE_MESSAGE) != SMS_DELIVER_ONE_MESSAGE) return -3;

	const int sender_number_length = buffer[sms_deliver_start + 1];
	if (sender_number_length + 1 > sender_phone_number_size) return -4;  // Buffer too small to hold decoded phone number.
	...
	if (sms_start + 1 > buffer_length) return -5;  // Invalid input buffer.

	const int output_sms_text_length = buffer[sms_start];
	if (sms_text_size < output_sms_text_length) return -6;  // Cannot hold decoded buffer.
	...
	if (decoded_sms_text_size != output_sms_text_length) return -7;  // Decoder length is not as expected.
